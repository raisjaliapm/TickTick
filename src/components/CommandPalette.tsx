...
export function CommandPalette({ tasks, onToggle }: CommandPaletteProps) {
  ...
  useEffect(() => { if (open) { inputRef.current?.focus(); setQuery(''); } }, [open]);
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ opacity: 0, y: -10, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.98 }}
          ...
          <div className="max-h-64 overflow-y-auto scrollbar-thin p-1">
            {filtered.length === 0 && <p className="text-sm text-muted-foreground text-center py-6">No tasks found.</p>}
            {filtered.map(task => (
              <button key={task.id} onClick={() => { onToggle(task.id); setOpen(false); }}
                ...
                <span className={task.status === 'completed' ? 'text-task-completed line-through' : 'text-foreground'}>{task.title}</span>
              </button>
            ))}
          </div>
        </motion.div>

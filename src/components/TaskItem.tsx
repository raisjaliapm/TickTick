  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`relative flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-[5px] border protocol-transition ${
          isCompleted
      ...
      {/* Priority dot */}
      <div className={`h-1.5 w-1.5 rounded-full shrink-0 ${priorityDot[task.priority]}`} />
      ...
      <div className="flex flex-1 items-center gap-3 min-w-0">
        {isEditing ? (
          ...
          <span
            onDoubleClick={() => { setIsEditing(true); setEditValue(task.title); }}
            className={`text-sm truncate protocol-transition ${
              isCompleted ? 'text-task-completed line-through' : 'text-foreground'
            }`}
          >
            {task.title}
          </span>
      ...
      <div className="opacity-0 group-hover:opacity-100 protocol-transition flex items-center gap-2 shrink-0">
        ...
        <button
          ...
          className="text-[11px] font-mono text-muted-foreground hover:text-destructive protocol-transition"
        >
          ⌫
        </button>
      </div>

      {/* Due date always visible if overdue */}
      {isOverdue && (
        <span className="text-[11px] font-mono text-priority-urgent shrink-0 group-hover:hidden">
          overdue
        </span>
      )}
